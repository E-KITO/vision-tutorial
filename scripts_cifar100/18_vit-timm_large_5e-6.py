...
def main():
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for images, labels in tqdm(testloader, desc=f"Epoch {epoch+1}/{epochs}"):
                images, labels = images.to(device), labels.to(device)
                outputs = model(images)
                loss = criterion(outputs, labels)
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), './output/best_vit-timm_cifar100_large_5e-6.pth')
